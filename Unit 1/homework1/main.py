...
class LoginHandler(Handler):
    ...
    def post(self):
        ...
        # else:
        check_pass = valid_pw(username,password,u.passhash)
...
##### PARSE A XML WE GET FROM A REQUEST TO ANOTHER API 



# def get_coords(xml):
    # p = minidom.parseString(xml)  # parse the string
#     coor = p.getElementsByTagName("gml:coordinates")  # we get the coordintes tag
#     if coor and coor[0].childNodes[0].nodeValue:  # check if the contetn is not empty
#         lon , lat = coor[0].childNodes[0].nodeValue.split(',')    # separete both lon and lat values
#         return lat, lon
#     ###Your code here
